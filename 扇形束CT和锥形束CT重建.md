...
# 等距离扇形束（Equal-Distance Fan Beam）CT
...
# 锥形束CT（Cone Beam CT）重建（FDK）

> !24 **Tuy的条件（锥形束数据量充足条件）：**每一个与物体相交的平面都必须包含至少一个锥形束的焦点位置。!
><div style="display:none"> ![Tuy condition](https://github.com/ffdd1818/images/blob/main/Tuy%E6%9D%A1%E4%BB%B6.jpg "锥形束数据完备条件")  </div>
> ![Tuy条件.jpg](https://docimg3.docs.qq.com/image/AgAABenPl4GLYXuKBStO37Ci3xpL9D-t.png?w=899&h=712)

> - FDK的锥形束图像重建算法是专门为锥形束的圆形焦点轨道而设计；
> - FDK认为非中心平面的投影数据，都可以看作，以射线源为支点倾斜一个角度，折算到中心平面的扇形束投影；
> - 由于圆圈轨道不满足 Tuy  条件，FDK算法只能提供一个近似的重建图像，特别是在远离轨道平面的区域，伪影表现得更明显。实际上，只有在轨道平面上，这个算法是个精确的。当重建平面远离中心平面时，重建误差逐渐增大，适当增加投影数目，有利于提高重建图像质量。

为了便于分析和实施FDK重建算法，引人虚拟探测器的概念。虚拟探测器位于重建物体的旋转中心轴上，且平行于真实探测器，$\color{red}Oxyz$表示被扫描对象坐标系，焦点（光源）和探测器的位置相对不变，一起绕着旋转中心$O$点（即坐标原点）旋转。
![锥束CT虚拟探测器.png](https://docimg7.docs.qq.com/image/AgAABenPl4H_WQFGBzJJ8aUCqmm5ybD_.png?w=627&h=483)
 <div style="display:none">![锥束CT虚拟探测器.png](https://github.com/ffdd1818/images/blob/main/%E9%94%A5%E6%9D%9FCT%E8%99%9A%E6%8B%9F%E6%8E%A2%E6%B5%8B%E5%99%A8.png) </div>
  $\color{red}Osz$为虚拟探测器平面，表示探测器投影数据坐标系；$\color{red}Ots$表示射线源坐标系。此外规定$\color{red}z$轴是旋转中心轴，$\color{red}t$轴总是通过射线源中心，且与探测器平面相垂直。射线源$\color{red}S_o$到坐标原点$\color{red}O$的距离$\color{red}\overline{S_oO}=D$，$\color{red}\beta$表示$s$轴与$x$轴之间的旋转角度，由坐标系转换关系可知：
 $$
\color{blue}\left[ {\begin{array}{*{20}{c}}
s\\
t
\end{array}} \right] = \left[ {\begin{array}{*{20}{c}}
{\cos \beta }&{\sin \beta }\\
...
\end{array}} \right]
$$
在坐标系$\color{red}Oxyz$中，${\boldsymbol{\vec r}}=(x,y,z)$表示重建空间的一点$A$，该向量还可以在坐标系$\color{red}Ostz$中表示为${\boldsymbol{\vec r}}=(s,t,z)$，二者的转换关系，可以通过上面的矩阵表示。

 ![锥束CT几何参数图.png](https://docimg4.docs.qq.com/image/AgAABenPl4G0FgTXbDFBX7cPYT_7rje4.png?w=1406&h=1023)
 
 通过$A$点的的射线$S_oA$和虚拟探测器平面$\color{red}Osz$必然有一交点，交点坐标为$\color{red}\left[s(\boldsymbol{\vec r}),z({\boldsymbol{\vec r}})\right]$，该交点与$s$轴相交于$\color{red}F$点，如图$(b)$所示，与$z$轴相交于$\color{red}K$点，如图$(c)$所示。此外，重建点$A$在$\color{red}Ost$平面的投影记为$\color{red}A^\prime_{Ost}$，如图$(b)$所示，在$\color{red}Otz$平面的投影记为$\color{red}A^{\prime\prime}_{Otz}$，在$\color{red}Osz$平面的投影记为$\color{red}A^{\prime\prime\prime}_{Osz}$。

由图$(b)$可知，射线$S_oA$与虚拟投影平面的**交点**$\color{red}\left[s(\boldsymbol{\vec r}),z({\boldsymbol{\vec r}})\right]$，**交点**的$s$轴坐标$\color{red}s(\boldsymbol{\vec r})=\overline{OF}$；由图$(c)$可知，**交点**的$z$轴坐标$\color{red} z(\boldsymbol{\vec r})=\overline{OK}$。此外，由几何关系可知，$\overline{OP} = \boldsymbol{\vec r} \cdot \boldsymbol{\vec t}, \quad \overline{PA^\prime_{Ost}}=\boldsymbol{\vec r} \cdot \boldsymbol{\vec s}, \quad  \overline{PA^{\prime\prime}_{Ozt}}=\boldsymbol{\vec r} \cdot \boldsymbol{\vec z}$。

在图$(b)$中，由三角形$S_oOF$和$S_oPA^\prime_{Ost}$相似，故
$$
\dfrac{\overline{S_oO}}{\overline{S_oO}+\overline{OP}}=\dfrac{\overline{OF}}{\overline {PA^\prime_{Ost}}} \quad \Rightarrow {\color{red}s(\boldsymbol{\vec r})=\overline{OF}}=\dfrac{D\cdot(\boldsymbol{\vec r} \cdot \boldsymbol{\vec s})}{D+\boldsymbol{\vec r} \cdot \boldsymbol{\vec t}}
$$
同理，在图$(c)$中，可以得到：
$$
\dfrac{\overline{OK}}{\overline{PA^{\prime\prime}_{Ozt}}}=\dfrac{\overline{S_oO}}{\overline{S_oO}+\overline{OP}} \quad \Rightarrow {\color{red} z(\boldsymbol{\vec r})=\overline{OK}}=\dfrac{D \cdot (\boldsymbol{\vec r} \cdot \boldsymbol{\vec z})}{D+\boldsymbol{\vec r} \cdot \boldsymbol{\vec t}}
$$

定义投影函数$\color{red}p_\beta(s,z)=p(s,z,\beta)$，于是FDK滤波反投影法的公式为：
$$
\bbox[#f9de9f,6pt]{
\begin{align}
f(x,y,z)&=\dfrac{1}{4\pi^2}\int_0^{2\pi}\dfrac{D^2}{(D+\overline{OP})^2}\left[\int_{-\infty}^{\infty}\dfrac{D}{\sqrt{D^2+s^2+z^2(\boldsymbol{\vec r})}}p(s,z,\beta) \cdot h(s^\prime-s)ds\right]d\beta  \\
&=\dfrac{1}{4\pi^2}\int_0^{2\pi}\dfrac{D^2}{(D+\boldsymbol{\vec r} \cdot \boldsymbol{\vec t})^2}\left[\int_{-\infty}^{\infty}\dfrac{D}{\sqrt{D^2+s^2+z^2(\boldsymbol{\vec r})}}p(s,z,\beta) \cdot h(s^\prime-s)ds\right]d\beta
\end{align}
}
$$
其中$\dfrac{D^2}{(D+\overline{OP})^2}$相当于扇形束的加权因子$\dfrac{1}{U^2}$；$\dfrac{D}{\sqrt{D^2+s^2+z^2(\boldsymbol{\vec r})}}=\cos\gamma \cos k$，当$z(\boldsymbol{\vec r})=0$时（**即中心平面**），退化为扇形束的公式，对于**非中心平面**通过扇束角$\gamma$、锥角$k$折算到中心平面，因此是不精确的重建。

> **注意：**锥形束的旋转角定义与扇形束旋转角定义是不同的，具体情况如下图所示，即$\beta_{cone} = -(180^\circ - \beta_{fan})$
> ![旋转角不同的定义.png](https://docimg6.docs.qq.com/image/AgAABenPl4F-_Q0B90tHibyZBa2DK_WY.png?w=1472&h=1078)
